...
async function clickPickerControl(page, id, nodeName) {
  ...
  try {
    ...
    const button = pickers[0].find(node => node.nodeName === nodeName && attribute(node, 'id') === id);
    assert.ok(button, `Missing picker control: ${id}`);
    assert.equal(attribute(button, 'disabled'), undefined, `Picker control is disabled: ${id}`);
    const { model } = await cdp.send('DOM.getBoxModel', { nodeId: button.nodeId });
    assert.ok(model.width > 0 && model.height > 0, `Picker control is not visible: ${id}`);
...
export async function clickPickerButton(page, id) {
  ...
  await clickPickerControl(page, id, 'BUTTON');
}

export async function fillPickerField(page, id, value) {
  assert.ok(['left', 'right', 'top', 'bottom'].includes(id), `Unknown picker field: ${id}`);
  assert.ok(Number.isFinite(value), 'Picker field value must be finite');
  await clickPickerControl(page, id, 'INPUT');
