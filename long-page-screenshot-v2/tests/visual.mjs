...
export async function testVisual({page,worker,capture,waitFor,PNG}) {
 for(const kind of ['static','github','github-recovery','github-recovery-small','sidebar','ad','islands','global','transient','unrelated','ambiguous','low']) {
  ...
  if(kind==='unrelated') {
   ...
   const left=['github','github-recovery','github-recovery-small','sidebar','ad','islands'].includes(kind)?270:0,right=kind==='islands'?790:900;
