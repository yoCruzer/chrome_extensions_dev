...
export async function testVisual({page,worker,capture,waitFor,PNG}) {
 for(const kind of ['static','github','sidebar','ad','islands','global','transient','unrelated','ambiguous','low']) {
  await page.goto(new URL('?kind='+kind,page.url()).href);
  const ref=PNG.sync.read(Buffer.from(await page.evaluate(()=>reference),'base64'));
  if(kind==='transient')await worker.evaluate(()=>{
   ...
   chrome.tabs.captureVisibleTab=async(...args)=>{
    ...
    if(calls===3)chrome.tabs.captureVisibleTab=original;
  ...
  if(['unrelated','ambiguous','low'].includes(kind)) {
   ...
   const left=['github','sidebar','ad','islands'].includes(kind)?270:0,right=kind==='islands'?790:900;
   ...
   if(['github','global'].includes(kind))assert.ok(v.trace.some(t=>t.correction===-(kind==='github'?45:80)),JSON.stringify(v));
   ...
   if(['github','sidebar'].includes(kind))assert.ok(result.diagnostics.fullProof.trace.some(t=>t.trigger==='WITNESS_MOVED'&&Math.abs(t.delta.dy-(kind==='github'?45:20.617))<0.1));
  }
