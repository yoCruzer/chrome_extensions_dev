...
for(const result of ['failed','ambiguous','low-information','matched'])test(`offscreen visual-first terminal ${result}`,async()=>{
 ...
 context.draws=draws;
 vm.runInNewContext(`session={id:'a',bitmapWidth:900,bitmapHeight:912,region:{x:0,y:0,width:900,height:6428},scale:{sourceX:1,sourceY:1,scaleX:1,scaleY:1},context:{drawImage(...a){draws.push(a.slice(1))}},previous:{strip:{start:428},canonicalY:5472,documentY:5472,end:6384}}`,context);
